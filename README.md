...
PaperSOTA 是一个面向数据集的论文、算法与 SOTA 指标调研 Skill，同时提供可直接运行的 CLI 脚手架。它适合从研究方向、论文标题/链接或明确的数据集名称出发，自动组织数据集验证、相关论文检索、指标抽取和 Markdown SOTA 报告生成。

默认报告语言为中文，表格列名和指标名保留英文。若用户全程使用英文提问，或明确要求英文，PaperSOTA 应输出英文报告。
...
PaperSOTA 是研究助理，不是最终权威来源。公开 API 可能遗漏论文，PDF 可能不可访问或无法可靠抽表，部分指标也可能只出现在补充材料、图像或外部 leaderboard 中。正式发表或引用前，请人工核对原论文、官方 benchmark 页面和补充材料。
